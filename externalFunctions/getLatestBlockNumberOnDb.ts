...
export const getLatestBlockNumberOnDb = async (
  tableName: string,
): Promise<number> => {
  console.log(
    `${currentTimeReadable()} | Start : Detecting the latest block number on the DB`,
  )
  ...
  if (queryResult.length === 0) {
    console.log(
      `${currentTimeReadable()} | End : Detecting the latest block number on the DB | The DB is empty | Process time : ${(
        (endTime - startTime) /
        1000
      )
        .toString()
    ...
    console.log(
      `${currentTimeReadable()} | End : Detecting the latest block number on the DB | Block number : ${
        queryResult[0].number
      } | Process time : ${((endTime - startTime) / 1000)
        .toString()
        .slice(0, -12)} sec`,
    )
